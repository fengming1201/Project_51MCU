...
* Arguments  : pevent        is a pointer to the event control block associated with the desired mutex
*
*              ppdata         is a pointer to a structure that will contain information about the mutex
*
...
#endif
    OS_ENTER_CRITICAL();
    ppdata->OSMutexPIP  = (INT8U)(pevent->OSEventCnt >> 8);
    ppdata->OSOwnerPrio = (INT8U)(pevent->OSEventCnt & OS_MUTEX_KEEP_LOWER_8);
    if (ppdata->OSOwnerPrio == 0xFF) {
        ppdata->OSValue = 1;
    } else {
        ppdata->OSValue = 0;
    }
    ppdata->OSEventGrp  = pevent->OSEventGrp;               /* Copy wait list                           */
